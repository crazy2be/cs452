...
#ifdef QEMU
	ASSERT(puts(COM1, "Changing train speed" EOL) == 0);
#else
	char train_command[3] = {speed - 1, train, 0};
	ASSERT(puts(COM1, train_command) == 0);
#endif
}
...
#ifdef QEMU
	ASSERT(puts(COM1, "Changing switch position" EOL) == 0);
#else
	char cmd = (d == STRAIGHT) ? 0x21 : 0x22;
	char sw_command[3] = {cmd, sw, 0};
	ASSERT(puts(COM1, sw_command) == 0);
#endif
}

void disable_switch_solenoid(void) {
#ifdef QEMU
	ASSERT(puts(COM1, "Resetting solenoid" EOL) == 0);
#else
	ASSERT(putc(COM1, 0x20) == 0);
#endif
}
